---
title: About Dev9b
description: "Learn about Dev9b, an open-source developer blog platform by F9XR Team."
slug: about
date: 2024-01-01
menu:
    main:
        weight: 5
        params:
            icon: user
---

## About Dev9b

**Dev9b** is a free, open-source online community where software developers share knowledge, write technical articles, and help each other grow.

### Our Mission

Dev9b exists to make developer knowledge accessible to everyone. We believe that sharing technical content openly helps the entire developer community grow faster.

### Who We Are

Dev9b is built and maintained by the **F9XR Team** — a group of developers passionate about open-source and knowledge sharing.

- **Website:** [f9xr.org](https://f9xr.org)
- **GitHub:** [github.com/f9xr](https://github.com/f9xr)
- **Email:** [hello@f9xr.org](mailto:hello@f9xr.org)
- **Contact:** Open an issue on our [GitHub repository](https://github.com/f9xr/dev9b)
- **Editorial standards:** See our [Editorial Policy](/editorial-policy/)

### Our Expertise

The F9XR Team brings hands-on experience across modern software development:

- **Web development** — HTML, CSS, JavaScript, and modern frameworks
- **Static site & Jamstack** — Hugo, static generators, GitHub Pages
- **Open source** — building and maintaining public repositories
- **Developer tooling** — CI/CD, automation, and best practices
- **Technical writing** — documenting code and teaching developers

Every article reflects real, first-hand engineering experience — not recycled marketing content. Read our [Editorial Policy](/editorial-policy/) to understand how we verify what we publish.

### What Makes Dev9b Different

| Feature | Dev9b | Other Platforms |
|---------|-------|-----------------|
| Free & Open Source | ✅ | Varies |
| Developer-focused | ✅ | Sometimes |
| Community reviewed | ✅ | Rarely |
| No ads | ✅ | No |
| GitHub-powered | ✅ | No |

### Technology

Dev9b is built with:

- [Hugo](https://gohugo.io/) — Static site generator
- [Hugo Theme Stack](https://github.com/CaiJimmy/hugo-theme-stack) — Clean, modern theme
- [GitHub Pages](https://pages.github.com/) — Free hosting
- [GitHub Actions](https://github.com/features/actions) — CI/CD automation

### License

All content on Dev9b is licensed under [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/) unless otherwise stated. The site source code is licensed under the [MIT License](https://github.com/f9xr/dev9b/blob/main/LICENSE).
